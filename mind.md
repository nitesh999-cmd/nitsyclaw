...
| 2026-04-28 | 5e | **Feature-request capture pipeline.** New `feature_requests` table (migration 0002), new shared `request_feature` tool wired to BOTH surfaces via `registerAllFeatures({surface})`, system prompt updated to route "add feature X" / "I want NitsyClaw to do Y" → tool. Daily CCR build agent will pick up pending rows, run NWP, implement, push, notify back via messages table. PARKED-TASKS.md retired (snapshot in deprecation note); `CLAUDE-CODE-BACKLOG.md` is now sole markdown backlog, `feature_requests` table is sole live queue. |
