...
| 2026-04-28 | daily-build-agent-1 | **Daily build agent first run — DB unreachable.** Agent booted successfully (all 6 files read, NWP acknowledged). Attempted DB connection to query `feature_requests` table. Blocked: `DATABASE_URL` not set in CCR routine environment and no `.env.local` present on the cloud runner. Zero pending rows processed. **Action required:** add `DATABASE_URL` (pooled Supabase URL from `.env.local` on the laptop) to the CCR routine's environment variables at https://claude.ai/code/routines/trig_01XiN9ZowcHufrXkcNzMkJbe — same var name used locally. Also add `GITHUB_PAT` if the git proxy isn't sufficient for push. Once env is set, next scheduled run (or manual trigger) will process pending rows automatically. See L32 below. |
