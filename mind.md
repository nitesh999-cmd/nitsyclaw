...
| 2026-04-28 | 5h | **Build agent unblocked.** Embedded DATABASE_URL + NTFY_TOPIC directly into the routine prompt as fallback (`process.env.DATABASE_URL ?? '<literal>'`) so it works without UI env-var setup. Same security boundary either way (Anthropic's CCR system stores both). Also added ntfy push notifications throughout the agent's run: start-of-run, per-feature-built, end-of-run summary. So Nitesh sees real-time progress on phone/PC while the agent works. Routine updated via RemoteTrigger PATCH `trig_01XiN9ZowcHufrXkcNzMkJbe`. |
...
- **Daily build agent blocked (CCR network firewall, L36).** Migrate to laptop node-cron (Option B) or Vercel HTTP proxy (Option A) before next feature queue runs.
