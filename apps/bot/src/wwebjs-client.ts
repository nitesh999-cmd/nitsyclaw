...
export class WwebjsClient implements WhatsAppClient {
  // Track recent outgoing message bodies. Any incoming with matching body within window = our own echo.
  ...
  constructor(private opts: WwebjsOptions) {
    ...
    this.client = new Client({
      ...
      puppeteer: { headless: true, args: PUPPETEER_ARGS, handleSIGINT: false },
      webVersionCache: {
        type: "remote",
  ...
  private isOurEcho(body: string): boolean {
    ...
    if (idx >= 0) {
      this.recentOutgoingBodies.splice(idx, 1); // consume the match
      return true;
    }
    return false;
  }
  ...
  private wireEvents(): void {
    ...
    const handleMessage = async (m: Message) => {
      try {
        ...
        // Echo guard: if this is fromMe and the body matches a recent outgoing, drop it.
        if (fromMe && this.isOurEcho(body)) {
          return;
        }
  ...
  async send(msg: OutboundMessage): Promise<{ id: string }> {
    // Track BEFORE sending so the echo arrives finding it already registered
