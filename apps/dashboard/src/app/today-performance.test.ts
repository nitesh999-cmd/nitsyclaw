...
describe("today dashboard performance guard", () => {
  it("uses a bounded storage timeout so login cannot buffer forever", () => {
    ...
    expect(source).toContain("Validation demo");
    expect(source).toContain("Not live in demo");
    expect(source).toContain("Best next action");
    expect(source).toContain("See connection status");
    ...
    expect(source).not.toContain('href: "/whatsapp-recovery"');
