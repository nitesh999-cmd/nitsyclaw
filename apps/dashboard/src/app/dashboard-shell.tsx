    ...
    <div className="nc-shell">
      <header className="nc-mobile-topbar">
        <a href="/" className="flex items-center gap-3">
          ...
          <span>
            ...
            <span className="block text-xs text-stone-600">Home admin</span>
          </span>
        </a>
      ...
      <aside className="nc-sidebar">
        <div className="mb-5">
          <a href="/" className="flex items-center gap-3">
            ...
            <span>
              ...
              <span className="mt-0.5 block text-xs text-stone-600">Personal life admin</span>
            </span>
          </a>
        ...
        <nav className="flex-1 space-y-5 overflow-y-auto text-sm">
          {navGroups.map((group) => (
            <div key={group.label}>
              <div className="mb-2 text-[11px] font-semibold uppercase tracking-normal text-stone-600">
                {group.label}
              </div>
              <div className="grid grid-cols-1 gap-1">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={active ? "w-10 text-[10px] font-semibold text-[#8e3f24]" : "w-10 text-[10px] font-semibold text-stone-600"}>{item.mark}</span>
          ...
          <form action="/api/auth/logout" method="post" className="border-t border-stone-200 pt-4">
            <button className="text-left text-sm text-stone-600 transition-colors hover:text-stone-950" type="submit">
              Sign out
            </button>
          </form>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 flex border-t border-stone-200 bg-[#fffdf8]/95 shadow-[0_-6px_24px_rgba(70,48,23,0.12)] backdrop-blur-xl lg:hidden">
          ...
          return (
            <a
              ...
              className={
                ...
                (active ? "text-[#8e3f24]" : "text-stone-600 hover:text-stone-950")
              }
            >
              {item.icon}
