  ...
  return (
    <div className="nc-page">
      <section className="nc-hero">
        <div className="nc-eyebrow">Long-term context</div>
        <h2 className="mt-2 text-3xl font-semibold">Memory</h2>
        <p className="mt-3 text-sm text-slate-400">{rows.length} entries. Long-term notes, facts, and pinned info.</p>
      </section>
      {rows.length === 0 ? (
        <section className="nc-section">
          <p className="nc-muted">No memories yet.</p>
        </section>
      ) : (
          ...
          {rows.map((r) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2 text-xs">
                <span className="nc-pill uppercase tracking-wide">
                  {r.kind}
                </span>
              ...
              {r.tags && r.tags.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {r.tags.map((t) => (
                    <span key={t} className="nc-pill min-h-6">
                      {t}
                    </span>
                  ))}
                </div>
