...
### R77 — A tool error is persisted as structured metadata, never as text (extends R76)
An error message is free text and routinely carries prose, URLs, queries, recalled memory, SQL, driver payloads, addresses, phone numbers, LIDs, request identifiers, encrypted content or credentials. None of it may reach `audit_log`. Durable storage receives only allowlisted structured fields: an error class, an optional error code drawn from a closed allowlist (never admitted by shape alone), and a validated Postgres SQLSTATE where one exists — never the SQL or driver text around it. The safe default for any unclassified failure is `errorClass: tool_error`; a tool may project a more specific allowlisted class or code, and a projection that returns anything outside the allowlists, or that throws, degrades to the generic class rather than leaking or failing the turn. The sanitized error string still reaches the model, the caller and the operational log unchanged: detailed sanitized logging and durable audit storage are separate channels and must stay that way.
- *Source:* Remaining free-text channel after R76 — `formatToolErrorText` output was written verbatim to `audit_log.error`; `packages/shared/src/agent/tool-error.ts`, `packages/shared/src/agent/loop.ts`
- *Added:* 2026-07-29
...
### R79 — A test that reads a repository file normalises line endings before asserting on them
Any test that loads a tracked file and asserts on its textual structure must normalise line endings first (`.replace(/\r\n?/g, "\n")`), unless the byte-level line ending is itself the property under test. Git materialises the same committed bytes as LF on Linux and CRLF on Windows under `core.autocrlf`, so an assertion anchored on `"\n  <token>:\n"` passes on one runner and fails on the other while the committed file is byte-identical. That failure is a property of the checkout, not of the repository content, and treating it as a content defect wastes CI cycles and erodes trust in a red build. Normalising is not a weakening: structural, ordering, indentation and condition assertions are unchanged; only checkout-specific EOL variance is removed. A test that genuinely needs to enforce an EOL policy must say so explicitly and belongs with a `.gitattributes` rule rather than an incidental structural assertion.
- *Source:* CI run 31807095414 at `eaf1b96` — the `windows` job failed `ci-workflow.test.ts` with "job e2e not found" while the identical `ci.yml` passed on `ubuntu-latest`; `ci-workflow.test.ts`
- *Added:* 2026-08-14
- *Corollary:* a test that *simulates* the other line ending must make the rewrite idempotent (`/\r?\n/`, never a bare `/\n/`), or it corrupts an already-CRLF checkout into `\r\r\n`

---

### R80 — An OS-specific behavioural test is platform-gated; its source-level guarantees are not
A test that executes a platform-specific helper — a PowerShell script, a Windows service, a shell built-in — must be gated with `describe.skipIf(process.platform !== "win32")` (or the equivalent) so it runs only where the helper exists. It must never be gated by deleting, weakening or loosening the assertion. The guarantees that can be checked without executing the helper — that it binds loopback only, that it never kills or pulls, that its exit codes are bounded, that callers degrade rather than fail — are *source* assertions and must stay cross-platform, because those are the properties a reviewer relies on and they hold on every runner. Splitting a suite this way is not a coverage loss: the behavioural block still runs on the OS that owns the behaviour, and the safety contract runs everywhere. A cross-platform CI matrix that executes an OS-only helper on the wrong OS produces a red build that says nothing about the code, which is the failure mode this rule exists to prevent.
- *Source:* CI run 31857444754 at `144ac15` — `ollama-recovery.test.ts > ensure-ollama behaviour` failed on `ubuntu-latest` because it drives the deliberately Windows-local `scripts/ensure-ollama.ps1`; the suite arrived from `fix/anthropic-web-search`, which had never been through CI, so the integration merge was the first time CI executed it; `ollama-recovery.test.ts`
- *Added:* 2026-08-14
- *Extends:* R2 (the local brain never blocks WhatsApp), R44 (one primary runtime)
- *Gap closed 2026-08-14:* the no-pull/no-remove verb check and the loopback-only / no-`0.0.0.0` check were lifted out of the gated block into the ungated `ensure-ollama safety contract` (`ollama-recovery.test.ts:180`, `:189`). Every source-level guarantee this rule names is now asserted on every runner; only PowerShell execution is gated.

---

### R81 — A frozen artifact is corrected by a recorded exception, never by a silent hash rewrite
Freeze manifests exist so that a changed byte is visible. When a frozen artifact must legitimately change — a security remediation, not a result revision — the correction is applied in place *and* accompanied by a `corrections` record carrying `previousSha256`, `correctedSha256`, every mathematically dependent aggregate before and after, the date, the reason, the behaviour-equivalence proof, the parent commit and Git blob that preserve the original bytes, and an explicit statement that this is an authorized exception rather than regeneration. Recomputing an aggregate is only permitted after the manifest's own algorithm has been shown to reproduce the pre-correction value exactly, so the new number is derived rather than asserted. The verifier itself is never weakened, removed or excluded, and the correction must be followed by a sabotage test proving the corrected manifest still detects an unauthorized change. Superseding a manifest (the V1.3 pattern) remains preferred where nothing live depends on it; an in-place recorded correction is for the case where a live verifier would otherwise be left failing, because a permanently red integrity gate teaches people to ignore integrity gates.
- *Source:* Semgrep `detect-non-literal-regexp` on `scripts/voice-eval/scoring-v2.1.ts:145`, frozen by `qwen3-asr-v15-scored.freeze.json` and `voice-smoke-v2.1-held-out.freeze.json`, both live test gates. Original bytes preserved at commit `c4d1ce5b4fd0d64d2ba336847d2a5808eef057a0`, blob `c794c82fedc4d7c341bf6f6e92299e941712ec26`, digest `945c1d31…82fb` → `b5765268…a3c9`
- *Added:* 2026-08-14
- *Follow-up, not addressed here:* `qwen3-asr-v15-scored.freeze.json` freezes `qwen3-asr-v15.test.ts`, the test that verifies it. That self-reference did not bite this time — the test needed no edit, which was proven before the manifests were touched — but a correction that did require editing that test would have no non-circular path. Worth restructuring so a manifest never freezes its own verifier.

### R82 — `pnpm-workspace.yaml` is the sole dependency-override authority
Dependency overrides live in `pnpm-workspace.yaml` and nowhere else. A `pnpm.overrides` block in `package.json` is prohibited, because pnpm accepts both and `package.json` wins silently. When both existed they drifted apart: edits to `pnpm-workspace.yaml` became inert while an `ip-address: 10.1.1` pin in `package.json` held a vulnerable version in place, and nothing failed to say so. One authority, permanently, enforced by a guard that asserts `pnpm.overrides` is absent from `package.json` and that the workspace override map matches an expected map exactly.
- *Source:* `pnpm-workspace.yaml`, `package.json`, `dependency-lock.test.ts` — "overrides live in pnpm-workspace.yaml only" and "declares exactly the expected override map"
- *Added:* 2026-08-16

### R83 — A dependency guard fails closed on anything it cannot prove safe (extends R82)
The guard's job is to refuse to report clean unless it can demonstrate clean, so every ambiguity resolves to failure:
- A lockfile key it cannot parse is a failure, never a skip. An ignored key is exactly how an unsafe version passes unnoticed.
- Parsing is structural and section-restricted — `packages:` and `snapshots:` only — so override ranges, importer paths, comments at any indentation and nested mappings cannot be mistaken for resolutions.
- Package names are validated as plain npm names, so a YAML anchor, alias or tag on a key cannot disguise a reintroduced package, and a scoped lookalike can never answer for the unscoped name.
- Every prerelease is refused, including one above the floor: only a release proves the advisory fix shipped.
- Floors are keyed per major line, and a major with no declared floor fails rather than passes, because an unvetted line has not been checked against any advisory.
- Every floor and the boundaries it must reject derive from one table, so lowering a floor cannot leave a stale negative test green.
- Every override must have a resolved-version policy or be required absent, so coverage cannot drift by hand.
Removal beats pinning where a package can be eliminated: `extract-zip` has no fixed version, so absence is the only safe state.
- *Source:* `dependency-lock.test.ts`. Established across six rounds of independent review, each of which found a real false-clean path: first-match-only Vite checking, prerelease read as its release, scoped impostors, substring override matching, unrestricted section scanning, annotated keys, comment-boundary truncation, and floors covering 5 of 16 remediated packages. Verified by 38 sabotage runs against poisoned copies
- *Added:* 2026-08-16

### R84 — Windows CI runs at half the core count, and a starved test is never mistaken for a slow one (extends R80)
Vitest defaults to roughly one worker per core. On the four-core Windows runner that over-subscribes the machine, because the suite contains Windows-only tests that shell out — `ollama-recovery.test.ts` drives `scripts/ensure-ollama.ps1` through about sixteen Windows PowerShell launches and is correctly given its own long budgets, while everything co-scheduled with it runs on the 5000ms default with the cores gone. `maxWorkers` is therefore capped to half the available parallelism on Windows and left at the default everywhere else. Before any timeout is raised, the test's real cost must be measured: if it is fast in isolation and fast on Linux, it is starved, not slow, and raising its budget hides the contention instead of fixing it. Only a test whose work is genuinely bounded and legitimately exceeds the default earns an explicit budget.
- *Source:* `vitest.config.ts` `resolveMaxWorkers`, guarded by `vitest-windows-concurrency.test.ts`. Windows CI failed three times on three unrelated tests, each a 5000ms timeout: at `c26ad2c` a SQLite test that runs in 37ms locally and 62ms on Linux, at `144ac15c` a freeze verifier that runs in 26ms, at `c4d1ce5b` a router test. Measured on a sixteen-core Windows machine, varying only the worker count, the slowest single test was 5817ms at one worker per core, 4363ms at a half, 3896ms at three eighths, 3523ms at an eighth; from a half downward every test still above 2500ms was an ollama test that already carried an explicit budget
- *Added:* 2026-08-16
...
|---|---|---|---|
| 2026-08-16 | The `windows` job stayed red after the dependency remediation, on a single 5000ms timeout — and the failure had moved across three unrelated tests over three runs (`voice-verifier-v1.2-repair.test.ts` at `c26ad2c`, `verify-voice-verifier-v1-freeze.test.ts` at `144ac15c`, `router.integration.test.ts` at `c4d1ce5b`), which ruled out a defect in any one of them. Raising the individual timeouts was the obvious move and would have been wrong | R84, R80 | Diagnosed before changing anything. The failing tests are not slow: the SQLite one runs in 37ms locally and 62ms on Linux, the freeze verifier in 26ms. Windows CI inflated them 135–190×. Cause is CPU starvation, proven by a controlled experiment on a sixteen-core Windows machine varying only `maxWorkers` — slowest single test 5817ms at one worker per core, 4363ms at a half, 3896ms at three eighths, 3523ms at an eighth, with zero failures at every level. The starving load is `ollama-recovery.test.ts`, which spawns about sixteen Windows PowerShell processes to exercise `ensure-ollama.ps1` (29.5s on CI, 11ms on Linux where it is correctly platform-gated) and already carries explicit per-test budgets of 60s to 180s, so it never fails itself — it just takes the cores. Repair is one Windows-only worker cap in `vitest.config.ts`; Linux keeps the default and its 3.91s run. From a half downward, every test still above 2500ms was an already-budgeted ollama test, so no timeout was raised, no test skipped, no assertion weakened, no retry added and no Windows coverage removed. `vitest-windows-concurrency.test.ts` guards the resolver against both platforms rather than the current one, so the Windows rule is verified on the Linux runner too; five sabotage runs confirm it fails under the diagnosed bad config, under one-worker-per-core, when the resolver is left unwired, when `Math.floor(1/2)` returns zero, and when Linux is capped needlessly |
| 2026-08-16 | `pnpm audit` reported 39 advisories and the `security` job blocked the branch. Diagnosis found two override lists — `pnpm.overrides` in `package.json` and `overrides:` in `pnpm-workspace.yaml` — that had silently diverged, with `package.json` winning and pinning a vulnerable `ip-address: 10.1.1` while edits to the workspace file did nothing | R82, R83 | Consolidated to one authority. 39 → 0 with no suppression, no ignore file and no threshold change. `extract-zip` (GHSA-jmr9-qjv8-65gv, high, no fixed version has ever shipped), `nodemailer` and `basic-ftp` removed from the tree entirely rather than pinned, which is strictly stronger. Floors pinned per advisory: ip-address 10.3.1, puppeteer 25.5.0, sharp 0.35.3, form-data 4.0.6, vite >=6.4.3 <7, plus the existing drizzle-orm, esbuild, qs, postcss, ws, brace-expansion 1.x/2.x/5.x, js-yaml and nanoid lines; `engines.node` raised to >=22.12.0 for Puppeteer 25. Runtime compatibility proven rather than assumed: Puppeteer 25.5.0 drives `whatsapp-web.js@1.34.7` to QR-ready despite its exact 24.38.0 pin, and Sharp 0.35.3 was proven on Vercel Linux x64 through the real Next 16.2.11 `/_next/image` optimizer — JPEG, PNG and WebP each HTTP 200 and decoded at 64×48 and 128×96, with `/api/sharp-proof` reporting `sharpVersion 0.35.3`, `platform linux`, `arch x64` — on a disposable Preview created with `vercel deploy` (never `--prod`), reached with the official `vercel curl` so Deployment Protection was handled by the CLI with no manual bypass token and no settings change, then deleted and verified gone with all pre-existing Previews intact. `dependency-lock.test.ts` rewritten as a fail-closed guard over six rounds of independent review, each of which found a real false-clean path; verified by 38 sabotage runs |
| 2026-08-14 | The `security` job ran for the first time on the integration branch and blocked on 30 Semgrep findings, and the `windows` job timed out one PDF router test. Investigation showed 28 of the 30 were **not** introduced by the branch: `main`'s own `ci.yml` carries the identical 23 mutable action tags, and `dependabot.yml` and `pnpm-workspace.yaml` are byte-identical to `main`, which last passed CI on 2026-08-04 — `--config auto` had since pulled newer supply-chain rules. Only two `detect-non-literal-regexp` findings were branch-new, and neither was exploitable | R79, R80, R81 | Supply-chain hardening applied repo-wide rather than treated as a branch defect: all 23 actions pinned to verified 40-character SHAs with the version retained in a comment; `blockExoticSubdeps`, `minimumReleaseAge: 10080` and `trustPolicy: no-downgrade` added (pnpm 10.33.2 accepts all three, lockfile unchanged); seven-day Dependabot cooldown on both ecosystems. The production regex now matches catalogue brands as a literal string, which is stronger than escaping because catalogue text never reaches the regex engine. **Length-preserving, safety-monotonic brand matching: ASCII case-insensitive; non-ASCII exact. May reject exotic Unicode case variants but cannot introduce a match rejected by the previous escaped `/iu` implementation.** Comparison runs code unit by code unit over equal-length windows, with no `toLowerCase`, `normalize` or any whole-string transformation, because case mapping is not length-preserving and a length change is precisely what lets two distinct strings collapse into a false match. Word boundaries stay exactly the old ones via a static `/^\w$/iu` test. Two earlier drafts were wrong and independent review caught both: the first lower-cased the haystack (U+0130 expands to two code units, shifting every later offset) and was looser on `ſSungrow`; the second still compared with `toLowerCase()` and was looser on a U+0130 expansion counterexample. Proven against the REAL escaped `/iu` oracle, run outside the repository: 100,000 deterministic fuzz cases plus explicit U+0130, U+017F, U+212A, sigma/final-sigma, sharp-S, combining-mark and surrogate-pair cases; 17,139 accepted matches, zero looser results, zero offset corruption. Documented stricter cases: U+017F against brand `s`, and U+212A against brand `k`. The frozen scorer was corrected under R81 with a full `corrections` record in both manifests. The router timeout is a runner-speed artifact, not a defect: PDF extraction alone is allowed `DEFAULT_PDF_PARSE_TIMEOUT_MS` (5s), the whole vitest default, so the test got the same 15s allowance `ea72f20` gave the adjacent PDF test. Semgrep 30 → 0; suite 2026 → green |
| 2026-08-14 | CI run 31857444754 at `144ac15` stayed red after the first EOL repair, on two remaining failures that the merge had surfaced rather than caused. `windows` failed `verify-voice-verifier-v1-freeze.test.ts`: its CRLF simulation did `fixtures.replace(/\n/gu, "\r\n")`, which on an already-CRLF checkout produces `\r\r\n` and diverges the hash (`564cf98e…` vs `12dbb1c8…`). `ubuntu-latest` failed `ollama-recovery.test.ts > ensure-ollama behaviour`, which executes the Windows-only `scripts/ensure-ollama.ps1` with no platform guard — the suite came from `fix/anthropic-web-search`, never previously exercised by CI | R79, R80 | Idempotent simulation `fixtures.replace(/\r?\n/gu, "\r\n")`, and `describe.skipIf(process.platform !== "win32")` on the behavioural block only. The ungated `ensure-ollama safety contract` and `recovery wiring` blocks read the script as source, so never-kill/stop/restart, detached hidden `serve` with no `-Wait`, the 0/10/11/12/13 exit-code contract, the cross-process lock and bounded timeouts, required-model ordering and caller degradation stay asserted on every runner. Two source assertions do **not** — the no-pull verb check and the loopback-only check share a `test()` with a PowerShell scenario, so they now run on Windows only (see R80 known gap). Both fixes verified against a real on-disk CRLF checkout of `voice-verifier-v1-fixtures.json` and `ci.yml`, restored byte-identical afterwards. Independent review also noted the CRLF equality assertion is tautological on an already-CRLF runner: it still proves CRLF-vs-LF equality on Linux, and the trailing-space negative assertion stays meaningful everywhere. Test-only: no production source, workflow YAML, freeze artifact, fixture, migration, schema, dependency or lockfile changed |
| 2026-08-14 | CI run 31807095414 at `eaf1b96` failed on the `windows` job with `job e2e not found: expected -1 to be greater than -1`, while `ubuntu-latest` parsed the same committed `ci.yml` without complaint. Cause was the checkout, not the workflow: `core.autocrlf` materialises `ci.yml` with CRLF on Windows, so `workflow.indexOf("\n  e2e:\n")` cannot match | R79 | `ci-workflow.test.ts` normalises the loaded text with `.replace(/\r\n?/g, "\n")` before asserting. Verified 15/15 under both an LF checkout and a simulated CRLF checkout, with `ci.yml` restored byte-identical afterwards. Test-only: no production code, workflow YAML, freeze artifact or migration changed. Two further failures at the same SHA remain open and are not addressed here — `verify-voice-verifier-v1-freeze.test.ts` (same CRLF class) and `ollama-recovery.test.ts` (no platform guard, PowerShell helper on `ubuntu-latest`) |
| 2026-08-01 | Nothing ensured the local Ollama server was running, so after a reboot or an unexpected exit the first private WhatsApp turn failed with `ModelRoutingError` and an apology reply. The router's refusal to escalate private data to a cloud model was correct; the missing piece was startup and recovery, which depended on Nitesh remembering a manual command | R2, R30, R44 | One reusable `scripts/ensure-ollama.ps1` called by `launch-bot.ps1` before the bot and by `broom.ps1` once per normal cycle, reusing the existing `PT2M` trigger with no new scheduled task. Probe-first so a healthy server causes zero starts; awaits an existing starting process rather than starting a second; exclusive cross-process lock plus a re-probe under the lock as two independent double-start guards; loopback-only `OLLAMA_HOST` scoped to the child with the parent restored in `finally`; never kills, restarts, pulls, installs or removes anything; bounded exit codes (0/10/11/12/13/1) with both callers degrading rather than failing, so a down local brain cannot stop, restart or delay a healthy WhatsApp bot; logs bounded status tokens only, asserted against a whitelist |
| 2026-07-31 | The WhatsApp runtime guard was one-directional: any Railway runtime was allowed unconditionally (a test even asserted it), so a Railway deploy could bring up a second whatsapp-web.js client while the laptop held the session, forcing a re-pair. Read-only diagnosis found the Railway container stuck before WhatsApp readiness for ~26 days — `/health` still serving the module-level default provider because `setHealthProvider` runs after an `await ready()` that never resolved. Railway's own source branch is `main` and its watchPatterns cover `apps/bot/**`, so a main push would deploy it | R2, R44 | One explicit non-secret setting `NITSYCLAW_WHATSAPP_RUNTIME_OWNER` (`laptop`\|`railway`, `z.enum().optional()`): Railway fails closed unless the value is exactly `railway`, enforced at `index.ts:39` before QR recovery, client creation, Chromium, session access, scheduler and message handling; laptop path unchanged and requires no new key in `~/.nitsyclaw/secrets/.env.local`; symmetric stand-down if ownership is assigned to Railway; CI `whatsapp-production-smoke` additionally gated on the matching repository variable so it skips by default while all other jobs stay untouched; Railway support, QR recovery and rollback retained as opt-in |
| 2026-07-30 | Removing a field from a durable payload silently broke its reader. 5431d9c took `ownerHash` out of `model_route` audit rows and recorded that no consumer read it, but `local-brain/page.tsx` filtered on exactly that key as JSON, so its "Last route" and "Reason" tiles matched nothing from that commit onward. The same commit also moved `reason` from output to input, leaving a second stale read (`output.reasonCode`) in the same file. `audit_log` is `global_operational` with `scopeColumn: null`, so simply dropping the predicate would have shown the newest row written by anyone as the signed-in owner's routing decision | R78, R41 | Audit query removed rather than widened; the page fails closed with "Owner-scoped routing telemetry is unavailable" and keeps live provider health, which needs no audit row; repo-wide scan test bans any JSON operator against `auditLog.input`/`auditLog.output` and any owner/tenant key read from audit JSON, with comments stripped so prose cannot satisfy it; a contract test pins the dashboard to the writer's exact key sets (input `mode`, `reason`, `requestClass`, `sensitivity`; output `route`, `model`, `fallback`) and proves two owners' rows are byte-identical |
| 2026-07-30 | `scripts/` is not a pnpm workspace package, so `pnpm -r typecheck` never reached `scripts/operator-runner.ts` or `scripts/operator-complete.ts` and the `SafeAuditEntry` brand was not compile-time enforced for two live audit writers. The gap also hid a real error: `operator-runner.ts` referenced an undeclared `EnvLike` type, which `tsx` strips without checking | R78 | Narrow `scripts/tsconfig.json` (`noEmit`, three files) plus `typecheck:scripts`, folded into root `pnpm typecheck` — which CI already runs — and into `release:check`; `EnvLike` declared locally to match the connector helpers; brand rejection in the scripts project proven by a temporary unbranded literal producing TS2345 |
| 2026-07-30 | Copying a request-supplied string field individually did not make it safe. `data_delete.exportSnapshotId` came straight off a form field, and its `UUID_RE` guard never matched the real producer, which mints `export_<14 digits>` — so the field was simultaneously unsafe by provenance and dead in practice. A wider sweep found the same class of hole in every builder: `scope`, `action`, `decision`, `nextStatus`, `status`, `mode`, `reason`, `requestClass`, `sensitivity`, `route`, `model`, `status`/`failureCode`, `timeoutCode` and the `tool` column were all copied unvalidated, the operator `tool` name was string-interpolated from a caller value, row-count map keys were copied raw, and `isoTimestamp` accepted any `Date.parse`-able string | R78 | `exportSnapshotId` removed and replaced with `hasExportSnapshot: boolean` (no audit consumer read the value; the receipt only needs the export-before-delete gate); `jobId` retained after proving all five evidence tests including DB `uuid().defaultRandom()` provenance; every remaining string now an exact-membership closed vocabulary, a compile-time-complete `AssertComplete` union, or a bounded normalization; row-count keys closed to the 12 real tables; timestamps gated on ISO-8601 shape plus range; operator tool names selected from a map instead of interpolated |
| 2026-07-30 | `data_delete` bypassed `logAudit` altogether with a direct `tx.insert(auditLog)` inside the erasure transaction, persisting a request-supplied `exportSnapshotId` and spreading the whole row-count tally | R78 | `auditDataDelete(...)` plus exported `safeAuditValues(...)` keep the transaction while routing through the contract; snapshot id kept only on shape match; counts copied per key, non-numeric values dropped; inventory test now scans `insert(auditLog)` too |
...
| 2026-07-29 | `model_route` audit input carried `ownerHash`, a stable owner-linked identifier, in durable storage across the bot and both dashboard producers | R76 | Shared `buildModelRouteAuditPayload(event)` builds the payload from the routing event alone; input is exactly mode/reason/requestClass/sensitivity and output exactly route/model/fallback; row-ownership uses of `ownerHash` untouched |
| 2026-07-29 | `errorCode` was admitted to `audit_log` by regex shape rather than an allowlist, so identifier- and secret-shaped tokens such as `customer_abc123` or `memory_secret_token` could still persist | R77 | Closed `AUDIT_TOOL_ERROR_CODES` vocabulary with a derived `AuditToolErrorCode` union; exact-membership check replaces the shape regex; out-of-vocabulary codes drop while class and SQLSTATE survive |
| 2026-07-29 | `audit_log.error` still held free-text tool error messages — prose, URLs, queries, SQL, identifiers — after the R76 payload fix | R77 | `tool-error.ts` classifies failures into allowlisted structured metadata; default `tool_error`; validated SQLSTATE kept as a bare code; runtime error string and operational logging unchanged |
