...
An error message is free text and routinely carries prose, URLs, queries, recalled memory, SQL, driver payloads, addresses, phone numbers, LIDs, request identifiers, encrypted content or credentials. None of it may reach `audit_log`. Durable storage receives only allowlisted structured fields: an error class, an optional short machine code matching a bounded lowercase shape, and a validated Postgres SQLSTATE where one exists — never the SQL or driver text around it. The safe default for any unclassified failure is `errorClass: tool_error`; a tool may project a more specific allowlisted class or code, and a projection that returns anything outside the allowlists, or that throws, degrades to the generic class rather than leaking or failing the turn. The sanitized error string still reaches the model, the caller and the operational log unchanged: detailed sanitized logging and durable audit storage are separate channels and must stay that way.
