...
- *Known gap, recorded rather than glossed:* the current split does not yet fully satisfy this rule. Two pure source assertions sit inside the gated block because they share a `test()` with a PowerShell scenario — the no-pull/no-remove verb check (`ollama-recovery.test.ts:114-115`) and the loopback-only / no-`0.0.0.0` check (`:122-126`). Those two no longer run on Linux. What still runs on every runner is the ungated `ensure-ollama safety contract` and `recovery wiring`: never kills/stops/restarts, starts only a detached hidden `ollama serve` with no `-Wait`, the stable 0/10/11/12/13 exit-code contract, the cross-process lock and bounded timeouts, required-model ordering with the marker written only on success, and caller degradation. Closing the gap means lifting those two assertions into the ungated block; it is deliberately not done in the same commit as the CI repair, so the repair stays reviewable as a pure runner fix.
